...
bool OS::findEmptyMemoryspace(Process* current_process, bitset<6> adress, vector<unsigned char>& ram)
{
	unsigned physical_idx = 0;
	for (size_t i = 0; i < ram.size(); i++){
		if (ram[i] == NULL){
			cout << "assigned page "<< adress.to_string()<< " to physical memory\n\n";
			for (size_t j = i; j < i+Process::OFFSET_LENGTH; j++){
			...
			current_process->getPageTable()[idx.to_ulong()][3] = (int)physical_idx;
